...
Status: **research + design options, nothing implemented.** This is a
reference document for a future session to pick up and execute against.
...
depths you can stop at. Suggested order, cheapest/safest first:

1. **§4.2** — wire `costProfile.model` into the executor. Independent,
   mechanical, no schema change.
2. **§3a** — show the handoff graph in the fleet view / `wissel why`.
   Independent, no schema change.
3. **§3b** — enforce handoffs for explicit follow-ups. Requires the
   `parentTaskId` schema decision — the first real design call in this
   list, make it deliberately rather than backing into it.
4. **§4.3** — fleet health status. Independent of 1–3, worth doing
   whenever there's appetite for a new small endpoint.
5. **§3c** — full structured auto-delegation. Gate this on the
   prompt-format work from the cost-reduction discussion landing first;
   don't build the parsing/follow-up-creation logic before agents
   actually emit something parseable.
6. **§4.1 / §4.4** — revisit only when the fleet or project count
   actually grows enough to hurt; not worth building against a
   hypothetical.

## 7. Open questions for whoever picks this up

- Does `parentTaskId` (§3b) get added now alongside the first handoff-
  enforcement work, or does 3a ship alone first and 3b waits for a
  separate decision? (Recommendation: separate decision — 3a has zero
  schema cost and shouldn't be blocked on settling 3b's design.)
...
- Is a `wissel fleet add --template` command (§4.1) worth building
  before or after the manifest genuinely outgrows one file? Recommend
  waiting — see §6.
